      ...
      <div className="flex-1 p-4 overflow-auto">
        <div className="columns-2 gap-3 space-y-3">
          {filteredImages.map((item) => (
            <div
              key={item.id}
              className={`${item.height} bg-gradient-to-br ${item.gradient} rounded-lg cursor-pointer hover:scale-[1.02] transition-transform shadow-sm break-inside-avoid mb-3`}
              title={item.name}
            />
          ))}
        </div>
